...
from .config import get_outdir, open_output_file
import toga
from toga.style import Pack
from toga.style.pack import COLUMN, ROW, CENTER, BOLD

# Main function to merge calendars
def merge(main_window, ics1_path, ics2_path, exclusions_path, excl_text, remove_text, merge_text):
    try:
        # Load the calendars from the input files
        cal1 = load_ics(main_window, ics1_path) if os.path.exists(ics1_path) else Calendar()
        cal2 = load_ics(main_window, ics2_path) if os.path.exists(ics2_path) else None
        ...
        if filtered_events:
            merge_text.value += f"There are {len(filtered_events)} new event(s) in ICS2 that do not exist in ICS1.\n\nThese event(s) are:\n\n"
...
# Run merge process
async def run_merge(self, main_window, ics1_path, ics2_path, exclusions_path):
    if not ics2_path:
    ...
    async def open_merge_results(widget):
        print("open_merge_results\n")
        save_path = get_outdir()
        try:
            if save_path:
                with open(save_path, 'w') as f:
                    f.write(output_cal.to_ical().decode("utf-8").replace('\r\n', '\n').strip())
            open_output_file(main_window, save_path)
        except Exception as e:
            main_window.info_dialog("Error", f"An unknown error occurred during opening: {e}")
    ...
    close_button = toga.Button('Close', on_press=lambda w: merge_window.close(), style=Pack(padding=5))
    ...
    # Run the merge
    output_cal = merge(main_window, ics1_path, ics2_path, exclusions_path, excl_text, remove_text, merge_text)
